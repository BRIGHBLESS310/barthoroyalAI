...
def google_search(q):
    try:
        r = requests.get(f"https://api.duckduckgo.com/?q={urllib.parse.quote(q)}&format=json&no_html=1", headers={"User-Agent": "Mozilla/5.0"}, timeout=8)
        data = r.json()
        ...
        snippet = soup.select_one('div.VwiC3b, div.BNeawe.s3v9rd, span.aCOpRe')
        if snippet:

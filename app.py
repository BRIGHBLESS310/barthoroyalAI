...
def google_search(q):
    try:
        ...
        ddg_url = f"https://api.duckduckgo.com/?q={urllib.parse.quote(q)}&format=json&no_html=1"
        r = requests.get(ddg_url, headers=headers, timeout=8)
        data = r.json()
        if data.get('AbstractText'):
            return f"{data['AbstractText']}\n\nSource: DuckDuckGo"

        g_url = f"https://www.google.com/search?q={urllib.parse.quote(q)}&hl=en"
        r2 = requests.get(g_url, headers=HEADERS, timeout=8)
        soup = BeautifulSoup(r2.text, 'html.parser')
        snippet = soup.select_one('div.VwiC3b, div.BNeawe.s3v9rd, span.aCOpRe, div.IsZvec')
        if snippet:
...
def smart_answer(q):
    ...
    return {"text": f"I searched for '{q}' - try different words!", "image": None}

UI = """
<!DOCTYPE html><html><head>
<title>cAI - Google + Wiki</title>
...
header{padding:12px 16px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #1a1a1a}
...
.card:hover{background:#1c1c1c;border-color:gold}
.bubble{padding:12px 16px;border-radius:20px;max-width:84%;margin:8px 0;white-space:pre-wrap;font-size:14.5px;line-height:1.5;position:relative}
...
.btn{color:#555;font-size:11px;border:1px solid #222;padding:5px 12px;border-radius:16px;background:transparent;cursor:pointer;margin-left:6px}
.btn:hover{color:#aaa;border-color:#444}
.btn-del{color:#ff5555;border-color:#ff555522}
.btn-del:hover{background:#ff555511}
...
<header>
<div style="display:flex;align-items:center;gap:8px"><img src="/logo.png" style="width:28px;height:28px;border-radius:50%" onerror="this.style.display='none'"><b style="color:gold">cAI</b></div>
<div style="display:flex">
  <button class="btn btn-del" onclick="deleteFirstText()">Delete First</button>
...
<footer>
<div class="box">
<input id="inp" placeholder="Search Google, Wikipedia or generate image..." onkeydown="if(event.key==='Enter')send()">
<button class="send" onclick="send()">↑</button>
</div>
</footer>
<script>
const SUGGESTIONS = [
 {icon:"🔍",t:"Google Search",d:"Latest news",p:"What is happening in Nigeria today?"},
 {icon:"🎨",t:"Create image",d:"Futuristic Lagos",p:"generate image of futuristic Lagos city"},
 {icon:"📚",t:"Wikipedia",d:"Benin History",p:"History of Benin Kingdom"},
 {icon:"💡",t:"Explain",d:"How AI works",p:"How does AI work?"}
];
function showWelcome(){
 let mots = ["What would you like to do today?","Search Google + Wikipedia + Generate Images ✨","Ready to create something amazing? 💡"];
 let mot = mots[Math.floor(Math.random()*mots.length)];
 let chat=document.getElementById('chat');
 chat.innerHTML = '<div class="welcome"><div class="mot">'+mot+'</div><h1>Good morning 👋</h1><p>Google search + Wikipedia + AI image generation.</p><div class="cards">'+SUGGESTIONS.map(s=>'<div class="card" onclick="quick(\\''+s.p+'\\')"><div>'+s.icon+'</div><div style=\\'font-weight:600;font-size:13px\\'>'+s.t+'</div><div style=\\'color:#666;font-size:11px\\'>'+s.d+'</div></div>').join('')+'</div></div>';
}
...
window.onload = ()=>{
 ...
 let saved = localStorage.getItem('cAI_final_all');
 if(saved && saved.length>60){ document.getElementById('chat').innerHTML = saved; }
 else { showWelcome(); }
};
function saveChat(){ if(!document.querySelector('.welcome')){ localStorage.setItem('cAI_final_all', document.getElementById('chat').innerHTML); } }
function clearChat(){ document.getElementById('chat').innerHTML=""; localStorage.removeItem('cAI_final_all'); showWelcome(); }
function deleteFirstText(){
  let chat = document.getElementById('chat');
  let bubbles = chat.querySelectorAll('.bubble');
  if(bubbles.length > 0){
    bubbles[0].remove();
    saveChat();
  } else {
    alert("No message to delete - chat empty!");
  }
}
async function send(){
 ...
 c.innerHTML+='<div class="bubble bot" id="temp">Searching Google & Wikipedia...</div>'; c.scrollTop=99999;
